    ...
    try {
      ...
      if (aiError.status === 429) {
        ...
        return res.status(429).json({
          ...
          retryAfter: aiError.errorDetails?.[2]?.retryDelay || '12s'
        });
      }
      
      // For other AI errors, try to provide a basic fallback
      console.log('AI detection failed, using fallback:', aiError.message);
      const fallbackType = 'General Contract'; // Basic fallback
      await redis.del(fileKey);
      return res.json({ 
        detectedType: fallbackType,
        note: 'AI detection failed, using fallback classification'
      });
    }
  } catch (error) {
...
export const detectAndConfirmContractType = async (
  ...
  try {
    ...
    try {
      ...
      await redis.del(fileKey); // Clean up Redis
      
      // If AI fails due to rate limits, return a clear error message
      if (aiError.status === 429) {
        const retryDelay = aiError.errorDetails?.[2]?.retryDelay || '60s';
        return res.status(429).json({
          message: 'AI service is temporarily unavailable due to rate limits.',
          error: 'Rate limit exceeded',
          retryAfter: retryDelay,
          suggestion: 'Please try again in a few minutes or upgrade your AI service plan.'
        });
      }
